...
<template>
  ...
  >
    <ToggleGroupItem
      ...
      :key="eye.name"
      :value="eye.name"
      class="group h-auto w-[50px] flex-col gap-1.5 rounded-[9px] px-0 py-1.5 hover:bg-transparent data-[state=on]:bg-chip lg:w-[66px] lg:gap-[7px]"
    >
      <span
      ...
      />
      <span
        class="font-mono text-[7.5px] font-normal whitespace-nowrap text-muted-foreground group-data-[state=on]:text-foreground lg:text-[9px]"
      >
        {{ eye.name }}
      </span>
    </ToggleGroupItem>
